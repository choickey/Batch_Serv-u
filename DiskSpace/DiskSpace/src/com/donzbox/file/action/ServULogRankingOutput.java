...
public class ServULogRankingOutput {
	...
	public void getRanking() {
		...
		ct = getContext(DN);
		// 다운로더가 없으면 [다운 더보기] 지워
		cnt = 0; for (int j=0 ; j<ct.length ; j++) {
			if (ct[j] == null) break; cnt++;
		}
		if(cnt == 0) {
			deleteDirectory(notice_root + REPORT_KIND_DN_MORE);
		}
		// 있으면 TOP RANK 와 더보기 생성
		else {
			for (int j=0 ; j<ct.length ; j++) {
				if (ct[j] == null) {
					break;
		...
		if(cnt == 0) {
			deleteDirectory(notice_root + REPORT_KIND_UP_MORE);
		}
		// 있으면 TOP RANK 와 더보기 생성
		else {		
			for (int j=0 ; j<ct.length ; j++) {
		...
		if(cnt == 0) {
			deleteDirectory(notice_root + REPORT_KIND_PO_MORE);
		}
		// 있으면 TOP RANK 와 더보기 생성
		else {		
			for (int j=0 ; j<ct.length ; j++) {
