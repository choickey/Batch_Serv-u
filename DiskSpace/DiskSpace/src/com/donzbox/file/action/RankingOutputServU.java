...
public class RankingOutputServU {
...
//				float fltDN = Float.parseFloat(m2g(strDN));
	...
	public RankingOutputServU(String notice_root) {
...
//			getRanking();
		...
		try {
			...
			while ((oneLineContents = in.readLine()) != null) {
				if (oneLineContents.contains(" " + (contextCnt) + ":")) {
					if (!(oneLineContents.toLowerCase().contains("[y]") || oneLineContents.contains("adult"))) {
	...
	public String [] getFilterContextUPDN(String context) {
		...
		for (String tmp : contextArray) {
			if (tmp.length()!=0 ) {
				if(cnt==1) returnValue[0] = tmp;
				if(cnt==4) returnValue[1] = tmp;
				if(cnt==7) returnValue[2] = tmp;
				cnt++;
			}
		}
		return returnValue;
